  ...
  <div class="header-right">
    <?php if ($user): ?>
      <?php if (($user['role'] ?? '') === 'faculty'): ?>
        <?php
        // Dynamically fetch assigned room for quick scanning
        $scan_link = BASE_URL . "views/faculty/assigned_rooms.php";
        if (isset($conn)) {
            $assigned_room_q = $conn->query("SELECT room_id FROM room_assignments WHERE faculty_id = " . (int)$user['id'] . " LIMIT 1");
            if ($assigned_room_q && $assigned_room_q->num_rows > 0) {
                $r_row = $assigned_room_q->fetch_assoc();
                $scan_link = BASE_URL . "views/faculty/view_room_assets.php?room_id=" . $r_row['room_id'] . "&auto_scan=1";
            }
        }
        ?>
        <a href="<?= $scan_link ?>" class="icon-btn" title="Scan Assets" style="margin-right: 8px;">
