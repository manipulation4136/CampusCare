...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($name) && $category_id > 0) {
        $stmt = $conn->prepare("INSERT INTO workers (name, category_id, contact) VALUES (?, ?, ?)");
        $stmt->bind_param("sis", $name, $category_id, $contact);
        if ($stmt->execute()) {
            set_flash('ok', 'Worker added successfully.');
            header("Location: " . BASE_URL . "views/admin/workers.php");
            exit;
        } else {
            set_flash('err', 'Failed to add worker.');
        }
    } else {
...
<div class="main-content">
    ...
    <div class="card animate-card-entry" style="max-width: 600px; margin: 0 auto;">
        <h2 class="card-title">Add New Worker</h2>
        ...
        <form method="POST">
            ...
            <div class="actions" style="text-align: right; margin-top: 20px;">
                <button type="submit" class="btn"><i class="fas fa-save"></i> Save Worker</button>
            </div>
        </form>
    </div>
</div>
