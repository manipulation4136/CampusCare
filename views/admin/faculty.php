...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['approve_user_id'])) {
        ...
        set_flash('ok', 'Faculty approved successfully');
    }
    // Delete User
    elseif (isset($_POST['delete_id'])) {
        ...
        if ($stmt->execute()) {
            set_flash('ok', 'Faculty deleted successfully');
        } else {
            set_flash('err', 'Failed to delete faculty');
        }
    }
    ...
    elseif (isset($_POST['update_user_id'])) {
        ...
        if ($name && $email) {
            ...
            if ($stmt->execute()) {
                set_flash('ok', 'Faculty updated successfully');
            } else {
                set_flash('err', 'Failed to update faculty settings');
            }
        } else {
...
$whereSQL = "WHERE role = 'faculty'";
$params = [];
$types = "";

if ($search) {
    $searchTerm = "%$search%";
    // Adjust based on your schema. Assuming faculty log in with email/register_id/name.
...
<div class="main-content">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
        <h2 style="margin: 0; color: #fff; font-size: 24px;">Faculty Management</h2>
        <div style="display: flex; gap: 12px; align-items: center;">
            <a href="<?= BASE_URL ?>views/admin/assignments.php" class="btn outline small">
                <i class="fas fa-tasks"></i> Assign Faculty
            </a>
            <a href="<?= BASE_URL ?>views/admin/forms/add_user.php?role=faculty" class="icon-btn-fab" title="Add New Faculty">
    ...
    <div class="table-card">
        ...
        <div class="table-header-row">
            <h3 class="card-title">All Faculty (<?= $totalUsers ?>)</h3>
            <form method="get" class="search-form">
                <input class="input search-input" name="search" placeholder="Search faculty..." value="<?= htmlspecialchars($search) ?>">
                <?php if ($search): ?>
                    <a href="faculty.php" class="btn outline small">Clear</a>
                <?php endif; ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display: flex; gap: 8px; align-items: center;">
                                        ...
                                        <button type="button" class="btn icon-btn" style="color: #6ea8fe; border: none; background: none; padding: 5px; cursor: pointer;" title="Edit Faculty"
                                            data-id="<?= $u['id'] ?>"
                                        ...
                                        <!-- Delete Action -->
                                        <form method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this faculty member? This action cannot be undone.');">
                                            <?= get_csrf_input() ?>
                                            <input type="hidden" name="delete_id" value="<?= $u['id'] ?>">
                                            <button type="submit" class="btn icon-btn" style="color: #e74c3c; background: none; border: none; padding: 5px;" title="Delete Faculty">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" style="text-align: center; padding: 40px; color: #8fa0c9;">
                                <?= $search ? 'No faculty found matching your search.' : 'No faculty found.' ?>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
...
<div id="editUserModal" class="modal" style="display:none; justify-content: center; align-items: center; z-index: 9999;">
    <div class="glass-card modal-content" style="max-width: 450px; width: 100%; border: 1px solid #2a3558;">
        <h3 style="color: #fff; margin-bottom: 20px;"><i class="fas fa-user-edit"></i> Edit Faculty Settings</h3>
